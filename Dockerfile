...
RUN mkdir -p /var/log/cart-service && chown -R ${USERNAME}:${USERNAME} /var/log/cart-service

# 작업 디렉토리 설정
WORKDIR /app

# builder 스테이지에서 빌드된 JAR 파일만 복사
COPY --from=builder /workspace/build/libs/*.jar cart-service-app.jar

# 파일 소유권을 non-root 사용자로 변경
RUN chown ${USERNAME}:${USERNAME} cart-service-app.jar
